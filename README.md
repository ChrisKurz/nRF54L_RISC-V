...
- [Let's do a first test: running _hello world_ on RISC-V](01_hello_world/README.md). This project runs in the Trustzone Secure domain. Default memory partition is used here (project is based on _Partition Manager_. Note that future _nRF Connect SDK_ versions will deprecate _Partition Manager_). 

- [Now, we run MCUboot in Secure domain, and the applications in Non-Secure domain](02_hello_world_ns/README.md). Default memory partition is used here (project is based on _Partition Manager_. Note that future _nRF Connect SDK_ versions will deprecate _Partition Manager_). 
